      ...
      db.serialize(() => {
        ...
        db.all(`SELECT posted_at, course_id FROM courses;`, (err, row) => {
          ...
          rev.forEach((ann) => {
            row.forEach((course) => {
              if (
                ann.context_code.replace("course_", "") ===
                  JSON.stringify(course.course_id) &&
                course.posted_at < ann.posted_at
              ) {
                ...
                db.all(
                  ...
                  (err, row) => {
                    ...
                    row.forEach((record) => {
                      if (
                        ann.context_code.replace("course_", "") ===
                        JSON.stringify(record.course_id)
                      ) {
                ...
                db.run(
                  `UPDATE courses SET posted_at=` +
                    JSON.stringify(ann.posted_at) +
                    ` WHERE course_id = ` +
                    JSON.stringify(course.course_id)
                ),
                  (err, row) => {
                    ...
                    console.log(
                      "Your records had been pushed to the DB succesfully!"
                    );
                  };
              }
            });
